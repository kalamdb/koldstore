...
//! columns, table DDL, primitive read/write SQL fragments, async batch planners,
//! and the PostgreSQL-free `pgoutput` decoder. Keep separate from
//! `koldstore-catalog`: catalog resolves *which* mirror a managed table uses;
//! this crate builds SQL *against* that mirror. Must stay a
