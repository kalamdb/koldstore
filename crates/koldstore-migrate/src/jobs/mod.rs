...
use koldstore_common::{SqlStatement, StorageId, TableName, TableOid};
...
impl MigrationBackfillJobRequest {
    ...
    /// # Errors
    ///
    /// Returns an error when the qualified table name cannot be re-validated as
    /// a [`TableName`].
    ...
    ) -> Result<Self, MigrationJobError> {
        let table_name = table
            .as_table_name()
            .map_err(|error| MigrationJobError::InvalidIdentifier(error.to_string()))?;
        Ok(Self {
            job_id,
            table_oid,
