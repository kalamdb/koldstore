...
fn migration_operation_lock_blocks_concurrent_table_conversion_work() {
    ...
    let plan =
        plan_migration_operation_lock(&table, koldstore_common::TableOid::from_raw(42)).unwrap();
...
fn migration_operation_lock_rejects_missing_table_oid() {
    ...
    assert!(
        plan_migration_operation_lock(&table, koldstore_common::TableOid::from_raw(0)).is_err()
    );
}
