...
#[test]
fn hot_update_stamp_advances_seq() {
    let stamp = DmlStamp::new(SeqId::new(10).unwrap(), ManagedDmlOperation::Update);
...
fn dml_stamp_marks_only_delete_operations_as_deleted() {
    for operation in [
        ...
        let stamp = DmlStamp::new(SeqId::new(1).unwrap(), operation);
        assert!(!stamp.deleted, "{operation:?} must keep the hot row live");
    }

    let delete = DmlStamp::new(SeqId::new(2).unwrap(), ManagedDmlOperation::Delete);
...
fn managed_update_effect_mutates_live_hot_row_and_records_mirror_update() {
    let effect = executor::plan_managed_update_effect(SeqId::new(10).unwrap());
...
fn seq_allocator_and_stamp_helper_assign_monotonic_row_effect_versions() {
    ...
    let stamp = stamp_dml_effect(second_seq, ManagedDmlOperation::Update);
