...
fn dml_scope_checks_run_before_heap_or_cold_metadata_access() {
    ...
    assert_eq!(
        koldstore::hooks::executor::enforce_dml_scope(TableKind::Shared, None, None,).unwrap(),
        None
    );

    assert_eq!(
        koldstore::hooks::executor::enforce_dml_scope(
            TableKind::User,
            Some(active.as_str()),
            Some(&row),
        )
    ...
    let missing_session =
        koldstore::hooks::executor::enforce_dml_scope(TableKind::User, None, Some(&row))
            .unwrap_err();
