...
use koldstore_mirror::{
    mirror_delete_using_selected_sql, quoted_pk_columns, selected_record_columns,
};
use thiserror::Error;

use crate::{
    migrate::QualifiedTableName,
    spi::{SpiStatement, SqlParamType},
};
...
"#,
    ...
    let statement =
        SpiStatement::write_with_params("clean schema flush cleanup", &sql, [SqlParamType::Jsonb])
            .map_err(|error| CleanupError::Spi(error.to_string()))?;
