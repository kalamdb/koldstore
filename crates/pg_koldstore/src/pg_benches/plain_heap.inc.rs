// Plain-heap baselines: same SQL shapes without manage_table, for overhead deltas.

fn prepare_plain_hot_table() {
    let _ = prepare_plain_messages("plain");
}

#[pg_bench(
    setup = prepare_plain_hot_table,
    transaction = "subtransaction_per_batch",
    sample_size = 30,
    measurement_time_ms = 2_000,
    warm_up_time_ms = 500
)]
fn plain_heap_insert_batch(b: &mut Bencher) {
...
fn plain_heap_count_scan(b: &mut Bencher) {
    let relation = ctx("relation");
...
fn plain_heap_pk_lookup(b: &mut Bencher) {
    let relation = ctx("relation");
...
fn plain_heap_update_by_pk(b: &mut Bencher) {
    let relation = ctx("relation");
    seed_rows(&relation, 1_000);
    b.iter(move || {
        Spi::run(&format!(
