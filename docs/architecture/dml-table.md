...
`internal_record_row_count_delta` (`pg_koldstore/src/sql/flush/counters.rs`)
calls `row_counter_cache::record_delta` once per capture statement (with the
statement row count), not once per heap row:
...
**Contract:** one manifest bump per touched table per transaction, not per row.
...
User SQL row (native PG types on heap)
  → Statement transition relation (new_rows / old_rows)
  → Set-based mirror write (ON CONFLICT / MERGE / direct UPDATE)
...
Capture is entirely trigger-based. There is no live `ExecutorStart` /
`ProcessUtility` DML-rewrite hook that writes the mirror.
...
[flushing-table.md](flushing-table.md), [scanning-table.md](scanning-table.md).
