...
| `cargo clippy --workspace --all-targets --all-features` | BLOCKED | pgrx rejects enabling multiple `pg$VERSION` features together. |
...
| `cargo nextest run --workspace --no-default-features --exclude e2e --exclude examples --exclude storage-comparison --exclude stress` | PASS | Workspace unit, integration, and doc tests passed; pgrx-backed E2E, examples, storage comparison, and chat penetration remain covered by their dedicated runners. |
...
| `scripts/run-all-tests.sh` | PASS | Full local verification: fmt, clippy, unit, pgrx compile/install, `#[pg_test]`, E2E (WAL-only), examples, storage, SQL, memory, benchmarks. |
