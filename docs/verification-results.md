...
| `cargo nextest run --workspace --no-default-features --exclude koldstore-e2e` | PASS | Workspace unit, integration, and doc tests passed; pgrx-backed E2E tests remain covered by the local E2E runner. |
| `tests/e2e/run_pg_matrix.sh` | PASS | Uses local pgrx PostgreSQL 16, installs `koldstore`, recreates `koldstore_pgrx_e2e`, and runs all E2E tests serially against the pgrx-managed server with no ignored tests. No Docker dependency. |
