...
manifest_paths="$(psql_cmd -tAc "SELECT count(*) FROM koldstore.manifest m JOIN pg_class c ON c.oid = m.table_oid JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'demo' AND m.generation > 0 AND m.sync_state = 'in_sync';" | tr -d '[:space:]')"
...
assert_eq "published manifests" "2" "${manifest_paths}"
