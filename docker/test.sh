...
null_system_columns="$(psql_cmd -tAc "SELECT (SELECT count(*) FROM demo.conversations WHERE _seq IS NULL OR _commit_seq IS NULL) + (SELECT count(*) FROM demo.messages WHERE _seq IS NULL OR _commit_seq IS NULL);" | tr -d '[:space:]')"
...
delete_events="$(psql_cmd -tAc "SELECT count(*) FROM koldstore.row_events e JOIN pg_class c ON c.oid = e.table_oid JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'demo' AND c.relname = 'messages' AND e.op = 'delete' AND e.deleted;" | tr -d '[:space:]')"
update_events="$(psql_cmd -tAc "SELECT count(*) FROM koldstore.row_events e JOIN pg_class c ON c.oid = e.table_oid JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'demo' AND c.relname = 'messages' AND e.op = 'update' AND NOT e.deleted;" | tr -d '[:space:]')"
...
assert_eq "null system columns" "0" "${null_system_columns}"
