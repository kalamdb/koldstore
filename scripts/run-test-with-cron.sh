...
Usage:
  scripts/run-test-with-cron.sh [options]
...
configure_pg_cron_preload() {
...

  {
    cat "${tmp}"
    echo
    echo "# koldstore pg_cron smoke (managed by scripts/run-test-with-cron.sh)"
    echo "${preload_line}"
...
Re-run:
  ...
  scripts/run-test-with-cron.sh --pg-version ${PG_VERSION}
  scripts/run-test-with-cron.sh --skip-prepare
EOF
